...
class compressDfFactory:
    def get(self, df: pd.DataFrame):
        df = df.rename(columns={
            ...
            'price_7d': 'pr7d',
            'percent_7d': 'pnt7d',
            'price_list_7d': 'prlst7d',
            'price_list_hy': 'prlsthy',
            'diff_24h': 'dif24h',
            'diff_7d': 'dif7d',
            'expansion': 'exp',
            'card_type': 'ctype',
                ...
                if 'archive' in durationData and durationData['archive'] is not None:
                    ...
                    if 'data' in archive and archive['data'] is not None:
                        ...
                        for dayData in archive['data']:
                            ...
                            l.append({
                                'dt': dayData['datetime'].replace(' 00:00:00',''), # datetime -> dt
                                'stock': dayData['count'],
                                'min': dayData['min'],
                                'p50': dayData['50%']
                            })
    ...
    def get7daysTopPrice(self, df: pd.DataFrame):
        ...
        for cname, item in dfPriceTops['price_list_7d'].iteritems():
            ...
            for i, dateData in enumerate(item['items']):
                dt = datetime.datetime.strptime(dateData['dt'], "%Y-%m-%d")
                #dt = datetime.datetime.strptime(dateData['datetime'], "%Y-%m-%d %H:%M:%S") #datetime -> dt
...
class priceChartDataProvider:
    ...
    def _getRecords(self, row: pd.Series, duration: str):
        ...
        for i, dateData in enumerate(item['items']):
            ...
            dt = datetime.datetime.strptime(dateData['dt'], "%Y-%m-%d") #datetime -> dt
            records[i]['date'] = dt.strftime("%Y-%m-%d")
...
compressFact = compressDfFactory()
compressDf = compressFact.get(priceDf)
ranks.save(compressDf,rank_dir+'/compact_all.json')
...
imageLoader.save(imageLoader.get(), deck_dir+'/deck_recipe.json')
imageLoader.only_poke = True
imageLoader.save(imageLoader.get(), deck_dir+'/deck_recipe_p.json')
